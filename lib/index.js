...
function pdftocairo( options, callback ) {
  ...
  if( (format = options.format) === undefined ) {
    throw new Error('pdftocairo: output format not specified')
  }

  if( (formatFlag = utils.outputFormatFlags[format]) === undefined ) {
    throw new Error('pdftocairo: output format not valid')
  }
  ...
  var onData = function(file, enc, cb ) {
    ...
    if( utils.isRaster( format ) ) {
      if( file.isStream() ) {
        throw new Error("gulp-pdftocairo: pdftocairo is buggy and doesn't stream raster formats :(")
        cb()
      } else {
